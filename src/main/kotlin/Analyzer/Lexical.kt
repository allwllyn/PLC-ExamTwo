...
class Lexical(viewModel: TokenViewModel){
    ...
    val variable = Regex("(\\$)([a-zA-Z_]{6,8})") //variable name / identifier
    ...
    val tokenMap = hashSetOf<String>("+","-","*","=","<=",">=","==","<",">","!=","!","{","}","[","]","(",")","~","dec","tiny","mid","big","as","being","fact","wrong")

    fun countTokens(strList: List<String>):Int{
        for(i in strList){
            if(tokenMap.contains(i)){
                tokenCount+=1
                continue
            }
